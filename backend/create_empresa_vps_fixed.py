...
Usando conexão correta com o container Docker
"""
import psycopg2
...
def create_empresa():
    # Configuração do banco de dados PostgreSQL na VPS (usando variável de ambiente do container)
    DATABASE_URL = "postgresql+psycopg2://agendamento:Ag3nd2026P0stgr3sS3cur3K3y@db:5432/agendamento_db"
    
    # Converter para psycopg2 normal
    ...
    try:
        ...
        # 1. Verificar se já existe plano ESSENCIAL
        cursor.execute("SELECT * FROM plans WHERE slug = %s", ("essencial",))
        plano = cursor.fetchone()
        
        if not plano:
            # Criar plano ESSENCIAL
            cursor.execute("""
                INSERT INTO plans (name, slug, description, price_monthly, max_professionals, 
                                 max_units, max_clients, max_appointments_per_month, 
                                 features, display_order, color, trial_days, is_active, is_visible,
            ...
            """, (
                "ESSENCIAL",
                "essencial",
                "Plano perfeito para pequenas empresas que estão começando",
                89.00,
                2,
                1,
                -1,
                -1,
                json.dumps(["clients", "services", "appointments", "financial_basic", "whatsapp_basic", "online_booking", "mobile_app"]),
                1,
                "#3B82F6",
                14,
                True,
        ...
        # 3. Criar empresa Studio Elegance
        cursor.execute("""
            INSERT INTO companies (name, slug, description, email, phone, website, 
                                 address, address_number, neighborhood, city, state, 
                                 postal_code, country, company_type, cnpj, trade_name, 
                                 whatsapp, business_type, team_size, timezone, currency, 
                                 primary_color, secondary_color, subscription_plan, 
                                 subscription_plan_id, business_hours, features, 
                                 online_booking_enabled, online_booking_description, 
                                 online_booking_social_media, is_active, created_at, updated_at)
        ...
        """, (
            ...
            "PJ",
            "12.345.678/0001-90",
            "Studio Elegance Beleza & Estética",
            "(11) 98765-4321",
            "salao_de_beleza",
            "2-5",
            "America/Sao_Paulo",
            ...
            True,
            "Agende seus serviços de beleza de forma rápida e conveniente",
            json.dumps({
                "instagram": "@studioelegance",
                "facebook": "StudioElegance",
                "tiktok": "@studioelegance.beleza"
            }),
            True,
            datetime.now(),
            datetime.now()
        ...
        # 4. Verificar se usuário admin já existe
        ...
        print(f"📋 Nome: {empresa['name']}")
        print(f"👤 Admin: andrekaidellisola@gmail.com")
        print(f"🔑 Senha: @DEDEra45ra45")
        print(f"🏢 ID da Empresa: {empresa['id']}")
        print(f"💼 Plano: {empresa['subscription_plan']}")
        print(f"💰 Preço: R$ {plano['price_monthly']}/mês")
