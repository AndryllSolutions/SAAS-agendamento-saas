  ...
  try {
    ...
    try {
      ...
      if (clients.data.length > 0) {
        const latest = clients.data[clients.data.length - 1];
        if (latest.email.includes('api')) {
          console.log(`   ✅ Último cliente via API: ${latest.full_name} (${latest.email})`);
        }
      }
    ...
    console.log('🔍 Verificando no banco de dados para confirmação final...');
