// Script de teste completo corrigido
const API_BASE = 'http://localhost:8000/api/v1';

async function testCompleteFlow() {
  console.log('🚀 Iniciando teste completo com PostgreSQL...\n');
  ...
  try {
    ...
    const registerData = {
      ...
      email: 'admin-dashboard@empresa-teste.com',
      phone: '11999999999',
      password: 'Senha123@',
      ...
      currency: 'BRL',
      team_size: '2-5',
      slug: 'empresa-teste-dashboard-2',
      plan_type: 'FREE',
      trial_end_date: null,
      referral_code: null,
      coupon_code: null,
    ...
    const loginResponse = await fetch(`${API_BASE}/auth/login`, {
      ...
      body: `username=${encodeURIComponent(registerData.email)}&password=${encodeURIComponent(registerData.password)}`
    });

    if (loginResponse.status !== 200) {
      const error = await loginResponse.json();
      console.log('❌ Erro no login:', error);
      console.log('Tentando login-json...');
      
      // Tentar login-json como fallback
      const loginJsonResponse = await fetch(`${API_BASE}/auth/login-json`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (loginJsonResponse.status !== 200) {
        console.log('❌ Erro no login-json:', await loginJsonResponse.json());
        return;
      }

      const loginData = await loginJsonResponse.json();
