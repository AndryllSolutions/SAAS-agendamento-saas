...
export default function NotificationsConfigPage() {
  ...
  const loadConfig = async () => {
    try {
      ...
      if (response.data) {
        ...
        setConfig(prev => ({
          smtp: { ...prev.smtp, ...(response.data.smtp || {}) },
          twilio: { ...prev.twilio, ...(response.data.twilio || {}) },
          whatsapp: { ...prev.whatsapp, ...(response.data.whatsapp || {}) },
          vapid: { ...prev.vapid, ...(response.data.vapid || {}) }
        }))
      }
      setLoading(false)
    } catch (error: any) {
  ...
  const testConnection = async (service: keyof NotificationConfig) => {
    ...
    try {
      ...
      setTestResults(prev => ({ ...prev, [service]: 'error' }))
      toast.error(error?.response?.data?.detail || `Erro ao testar conexão ${service}`)
    } finally {
      setTesting(null)
    }
  }
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type={showPasswords.vapid_private_key ? 'text' : 'password'}
